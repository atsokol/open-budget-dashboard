...
// Outputs a small Arrow IPC stream for index.md reverse subsidy breakdown.
...
const COD_CONS_MB_PK     = new Array(n);
const COD_CONS_MB_PK_NAME = new Array(n);
...
for (let i = 0; i < n; i++) {
  ...
  COD_CONS_MB_PK[i]      = String(r.COD_CONS_MB_PK);
  COD_CONS_MB_PK_NAME[i] = r.COD_CONS_MB_PK_NAME ?? "";
  ZAT_AMT[i]             = r.ZAT_AMT ?? 0;
  PLANS_AMT[i]           = r.PLANS_AMT ?? 0;
  FAKT_AMT[i]            = r.FAKT_AMT ?? 0;
}

const table = tableFromArrays({ CITY, REP_PERIOD, FUND_TYP, COD_CONS_MB_PK, COD_CONS_MB_PK_NAME, ZAT_AMT, PLANS_AMT, FAKT_AMT });
