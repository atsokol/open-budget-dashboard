// Data loader: incomes.parquet → Arrow IPC stream
// Eliminates parquet-wasm (6.4 MB) from the browser; apache-arrow (204 KB) is used instead.
...
const rows = await parquetReadObjects({ file: await asyncBufferFromFile(join(__dirname, "incomes.parquet")) });
const n = rows.length;

const CITY = new Array(n);
const REP_PERIOD = new Float64Array(n);
const FUND_TYP = new Array(n);
const COD_INCO = new Float64Array(n);
const NAME_INC = new Array(n);
...
for (let i = 0; i < n; i++) {
  ...
  COD_INCO[i] = r.COD_INCO;
  NAME_INC[i] = r.NAME_INC ?? null;
  ZAT_AMT[i] = r.ZAT_AMT ?? 0;
  PLANS_AMT[i] = r.PLANS_AMT ?? 0;
  FAKT_AMT[i] = r.FAKT_AMT ?? 0;
}

const table = tableFromArrays({ CITY, REP_PERIOD, FUND_TYP, COD_INCO, NAME_INC, ZAT_AMT, PLANS_AMT, FAKT_AMT });
