// Data loader: expenses-functional-economic.parquet → Arrow IPC stream
// Combined economic (COD_CONS_EK) × functional (COD_CONS_MB_FK) expense cross-classification
...
const COD_CONS_MB_FK = new Float64Array(n);
...
for (let i = 0; i < n; i++) {
  ...
  COD_CONS_MB_FK[i] = r.COD_CONS_MB_FK;
  ZAT_AMT[i]        = r.ZAT_AMT ?? 0;
  PLANS_AMT[i]      = r.PLANS_AMT ?? 0;
  FAKT_AMT[i]       = r.FAKT_AMT ?? 0;
}

const table = tableFromArrays({ CITY, REP_PERIOD, FUND_TYP, COD_CONS_EK, COD_CONS_MB_FK, ZAT_AMT, PLANS_AMT, FAKT_AMT });
